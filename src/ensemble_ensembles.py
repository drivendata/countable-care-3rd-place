...
def ensemble_feature(valid_file, label_file, test_file, feature_dir,
                     feature_name, combined_feature_name):
    # Load data files
    ...
    for i in range(label.shape[1]):
        ...
        combined_train_feature_file = os.path.join(feature_dir,
                                                   '{}.trn{:02d}.sps'.format(combined_feature_name, i))
        combined_test_feature_file = os.path.join(feature_dir,
                                                  '{}.tst{:02d}.sps'.format(combined_feature_name, i))

        idx = [x for x in range(val.shape[1]) if (x - i) % label.shape[1] != 0]

        X, y = load_svmlight_file(train_feature_file)
        dump_svmlight_file(sparse.hstack((X, val[:, idx])), y,
                           combined_train_feature_file, zero_based=False)

        X, y = load_svmlight_file(test_feature_file)
        dump_svmlight_file(sparse.hstack((X, tst[:, idx])), y,
                           combined_test_feature_file, zero_based=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--combined-feature-name', required=True,
                        dest='combined_feature_name')
    ...
    ensemble_feature(valid_file=args.valid,
                     ...
                     combined_feature_name=args.combined_feature_name)

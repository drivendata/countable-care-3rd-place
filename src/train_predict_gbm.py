...
def train_predict(train_file, test_file, predict_valid_file, predict_test_file,
                  n_est, depth, lrate, n_fold=5):

    logging.basicConfig(format='%(asctime)s   %(levelname)s   %(message)s',
                        level=logging.DEBUG, filename='gbm_{}_{}.log'.format(
                                                        n_est, depth
                                                       ))
    ...
    clf = GBM(n_estimators=n_est, max_depth=depth, learning_rate=lrate,
              random_state=2015)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--depth', default=4, type=int, dest='depth')
    parser.add_argument('--learn-rate', default=0.01, type=float, dest='lrate')

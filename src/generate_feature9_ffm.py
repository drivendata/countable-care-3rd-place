...
def generate_feature(train_file, test_file, feature_dir, name):
    # Load data files
    ...
    label = OneHotEncoder().fit_transform(
                np.reshape(LabelEncoder().fit_transform(trn.target.values),
                           (trn.shape[0], 1))
            )
    label = label.todense()
    ...
    for i, col in enumerate(cols, start=1):
        label_encoder = get_label_encoder(df.ix[:n_trn, col], min_obs=2)
        feature = df[col].apply(lambda x: label_encoder.get(x, 0))
        feature += 1
        feature[feature < 0] = 0

        df[col] = df[col].apply(lambda x: ' {}:{}:1'.format(i, x) if x > 0 else '')
